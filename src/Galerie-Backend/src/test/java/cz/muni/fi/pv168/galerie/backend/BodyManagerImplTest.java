...
/**
 * Example test class for {@link AlbumManagerImpl}.
 *
 * @author petr.adamek@bilysklep.cz
 */
public class BodyManagerImplTest {

    private AlbumManagerImpl manager;
    ...
    public void setUp() throws SQLException {
        ...
        manager = new AlbumManagerImpl(prepareClockMock(NOW));
    ...
    //--------------------------------------------------------------------------
    // Tests for AlbumManager.updateBody(Body) operation
    //--------------------------------------------------------------------------
    ...
    //--------------------------------------------------------------------------
    // Tests for AlbumManager.deleteBody(Body) operation
    //--------------------------------------------------------------------------
    ...
    //--------------------------------------------------------------------------
    // Tests if AlbumManager methods throws ServiceFailureException in case of
    ...
    // Now we want to test also other methods of AlbumManager. To avoid having
    // couple of method with lots of duplicit code, we will use the similar
    // approach as with testUpdateBody(Operation) method.

    private void testExpectedServiceFailureException(Operation<AlbumManager> operation) throws SQLException {
        SQLException sqlException = new SQLException();
        DataSource failingDataSource = mock(DataSource.class);
        when(failingDataSource.getConnection()).thenThrow(sqlException);
        manager.setDataSource(failingDataSource);
